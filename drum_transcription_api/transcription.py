...
from .onset_refinement import refine_predictions
...
class DrumTranscriber:
    ...
    def transcribe_to_midi(
        ...
        Args:
            ...
            refine_with_onsets: Refine predictions using audio onset detection (default: False)
            refinement_threshold: Max distance (sec) to snap to detected onset (default: 50ms)
            onset_delta: Onset detection sensitivity parameter (default: 0.05)

        Returns:
            Dictionary with statistics (includes 'refinement' key if refine_with_onsets=True)
        """
        drums_path = None
        temp_dir = None
        ...
        try:
            ...
            # Optionally refine onsets using audio-based onset detection
            refinement_stats = None
            if refine_with_onsets:
                logger.info("Applying onset refinement...")
                onsets, refinement_stats = refine_predictions(
                    audio_to_transcribe,
                    onsets,
                    snap_threshold=refinement_threshold,
            ...
            if refinement_stats:
                result["refinement"] = refinement_stats
