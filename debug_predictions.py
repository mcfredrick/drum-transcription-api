...
def main():
    ...
    for i, drum in enumerate(drum_names):
        ...
        print(f"{drum:8s}: min={class_preds.min():.3f}, max={class_preds.max():.3f}, mean={class_preds.mean():.3f}")
        print(f"         Frames > 0.5: {(class_preds > 0.5).sum()}, > 0.7: {(class_preds > 0.7).sum()}, > 0.9: {(class_preds > 0.9).sum()}")
